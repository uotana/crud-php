            ...
            <main>
                <div id="atualizar">
                    ...
                    if($_SESSION["estaLogado"]== true){
                        ...
                        echo "<div class=\"title\"><h1>Atualizar usuário</h1></div>";
                        
                        if ($result->num_rows > 0) {
                          // output data of each row
                            $row = $result->fetch_assoc();
                            
                            echo "<form class=\"big-form\" action='atualizar.php' method='post'>";
                            echo "<input type='hidden' name='id' value='".$id."'>";
                            
                            echo "<div>
                                    <label for=\"nome\">Nome: </label><br> 
                                    <input type='input' name='nome' autofocus value='".$row["nome"]."'>
                                </div>";
                            echo "<div>
                                <label for=\"sobrenome\">Sobrenome: </label><br> 
                                <input type='input' name='sobrenome' value='".$row["sobrenome"]."'>
                            </div>";
                            echo "<div>
                                    <label for=\"email\">Email: </label><br> 
                                    <input type='input' name='email' value='".$row["email"]."'>
                                </div>";
                            echo "<div><label for=\"senha\">Senha: </label>
                                    <input type='password' name='senha' value=''></br>
                                </div>";
                                
                            echo "<div class=\"buttons\">";
                            echo "<div>
                                        <button type='submit' name='opcao' value='atualizar'>Atualizar</button>
                                    </div>";
                            echo "<div>
